...
We discussed
   ...
   * Bias $E\hat{\Theta} - \theta$ and variance of point estimates
   
   * Estimation using maximum likelihood $L(\theta)$
...
Today we will discuss:
   ...
   * `R` Lab about data transformations and wrangling

---

# Reminder (2) motivation for point estimates

   * We want to be able to represent a population parameter via a statistic (estimator)
   
   * This will help us estimate the population parameter's value
   
   * We used the maximum likelihood method for that:
   
      * Assume a density function $f(x_i,\theta)$ or distribution function $P(x_i,\theta)$.
      
      * Represent the sample liklihood $L(x_1,\ldots,x_n,\theta)$ as a multiplication of the functions.
      
      * Use $\frac{d\ln(L(\theta))}{d\theta}$ to find the value of $\theta$ which yields the maximum value
      
      * Make sure you express $\theta$ as a function of $x_1,\ldots,x_n$.
...
# Method of Moments - Example - the log-Normal disribution

A random variable $X$ is *log-normally* distributed if $\ln(X)\sim\mathcal{N}(\mu,\sigma^2)$.
The mean and variance of $X$ are given by:

   * $\mu_X = EX=e^{\mu+\frac{\sigma^2}{2}}$
   * $\sigma_X^2 = \operatorname{Var}(X)=(e^{\sigma^2} - 1)e^{2\mu + \sigma^2}$

Let $X_i, i=1,\ldots,n$ be a sample taken from a log-normal distribution. Use the method of moments to find estimates for $\mu_X$, $\sigma_X$, $\mu$, and $\sigma$. Make sure that in your final answer you only use expressions such as $x_i$ and $\bar{x}$.

---

# Method of Moments - Example - the log-Normal disribution - Answer

$$\hat{\mu}_X = EX=e^{\mu+\frac{\sigma^2}{2}} = \bar{X}, \\ \hat{\sigma}_X^2 =  (e^{\sigma^2} - 1)e^{2\mu + \sigma^2} = \frac{\sum{(X_i-\bar{X})^2}}{n}$$
--

$$\bar{X}^2=e^{2\hat\mu + \hat\sigma^2}$$

--

$$\hat{\sigma}_X^2 = (e^{\hat\sigma^2}-1)e^{2\hat\mu + \hat\sigma^2} = (e^{\hat\sigma^2}-1)\bar{X}^2$$

--

$$\hat{\sigma} = \sqrt{\ln\left(\frac{\hat{\sigma}_X^2}{\bar{X}^2} + 1\right)}$$
--

$$\hat{\mu} = \ln(\bar{X}) - 0.5\ln\left(\frac{\hat{\sigma}_X^2}{\bar{X}^2} + 1\right)$$
