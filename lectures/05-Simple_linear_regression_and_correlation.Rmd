...
We talked about two sample statistics
   ...
   * Independence test (contingency table), using $\chi^2$.
...
The project should be submitted in and Rmd format, with code and data, by June 10th.
...
# Simple Linear Regression רגרסיה לינארית פשוטה
...
The regression coefficient $\beta_0$ is called the **intercept חותך**, and $\beta_1$ is called the **slope שפוע** (why?).
...
Where $\epsilon$ is assumed to be distributed as $\mathcal{N}(0, \sigma_\epsilon)$, we will also assume it is **homoscedastic הומוסכדסטיות**.

--

This model is called a **simple linear regression model רגרסיה לינארית פשוטה**.
...
The most common method to find the linear relationship is called the **least squares estimate שיטת הריבועים הפחותים**. I.e., we are looking for the line which brings to minimum the squared errors. I.e.: 

   * The $\min\sum_i(\hat{y}_i-y_i)^2$ of the red lines in :

```{r least squares example, echo=FALSE, fig.dim=c(6,5), message=FALSE, warning=FALSE}
...
To estimate $\sigma_\epsilon^2$ we can use $SS_E$ (proof out of scope):
...
Both coefficients are unbiased, i.e., $E[\hat{\beta}_0]=\beta_0$ and $E[\hat{\beta}_1]=\beta_1$ (proof [here](https://www.youtube.com/watch?v=LrvdZQ6jcZQ)).
...
# Prediction intervals רווחי תחזית
...
# Prediction intervals - illustration *[(Link to live demo!)](https://sarid.shinyapps.io/intervals_demo/)*
...
.right-plot[
```{r the actual plot, echo=FALSE, fig.dim=c(4,4), message=FALSE, warning=FALSE}
ggplot(wildlife_small, aes(x = rounded_height, y = n)) + 
   geom_point() + theme_bw() + 
   xlab("Height [k feet]") + 
   geom_line(data = wildlife_predicted, 
