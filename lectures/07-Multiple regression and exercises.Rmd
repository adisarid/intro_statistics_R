...
# Interactions - אינטראקציות בין משתנים
...
Set $l=$length, $d=$depth, Chinstrap=Species is Chinstrap (0/1), Gentoo=Species is Gentoo (0/1).
...
# Multicolinearity - מולטיקולינאריות

When using multiple variables we need to check for correlations between the explanatory variables (the $x$'s), otherwise we might get misleading results.

See example:

```{r example for multicolinearity, eval=FALSE}
set.seed(42)
...
Let's move to a real example - `penguins`, only the Gentoo speceis.

.small[
```{r penguins correlation, fig.dim=c(5, 5)}
penguin_cor <- penguins %>% 
   filter(species == "Gentoo") %>% 
   select_if(is.numeric) %>% select(-year) %>% 
   cor(use = "pairwise.complete.obs")

ggcorrplot::ggcorrplot(penguin_cor, lab = T)
```
]

---

# Handling multicolinearity with VIF

The variance inflation factor, or VIF for short:

$$\operatorname{VIF}_j=\frac{1}{1-R^2_j}$$

Where $R^2_j$ is the $R^2$ for a regression model where $X_j$ is the dependent variable and the rest are independent variables.
...
gentoo_lm <- lm(bill_length_mm ~ bill_depth_mm + flipper_length_mm + body_mass_g, data = gentoo)
...
A "rule of thumb" is applied to VIF (to see if $\operatorname{VIF}>5$). 

--

If $\operatorname{VIF}<5$, we can keep all variables.

---
 
# Variable Selection via Stepwise Regression - בחירת משתנים עם רגרסיה בצעדים
...
**The forward selection**: A similar algorithm, instead of removing, add variables one at a time

--

**The backward-forward**: Combination, at each iteration check both removal and addition.
...
A random sample of $n=100$ car owners show that in the state of Virginia, a car drives 23,500 km per year with a sample standard deviation of 3900 km.
   ...
   a. Construct a 95% confidence interval for the average number of kilometers a car is driven annually in Virginia.
   
   b. What can we assert with a 95% confidence interval about the possible size of our error if we estimate the average number of kilometers driven by car owners in Virgina to be 23,500 km?
   
   c. Danny has just moved to Virginia, provide a prediction interval at a 95%  to the amount of kilometers Danny will drive in the next year.
...
A random sample of $n=100$ car owners show that in the state of Virginia, a car drives 23,500 km per year with a sample standard deviation of 3900 km.
   ...
   a. Construct a 95% confidence interval for the average number of kilometers a car is driven annually in Virginia. `23500 + c(-1, 1)*qt(p = 0.975, df = 99)*3900/sqrt(100)`.
   
   b. What can we assert with a 95% confidence interval about the possible size of our error if we estimate the average number of kilometers driven by car owners in Virgina to be 23,500 km? *Up to 1550 km*.
   
   c. Danny has just moved to Virginia, provide a prediction interval at a 95%  to the amount of kilometers Danny will drive in the next year. `23500 + c(-1, 1)*qt(p = 0.975, df = 99)*3900*sqrt(1 + 1/sqrt(100))`
...
Assume the populations are normally distributed with equal variances. Use a p-value to report you conclusions.
.small[
$$H_0: RMR_{running} = RMR_{sedentary}$$
$$H_1: RMR_{running} > RMR_{sedentary}$$

$$T = \frac{\bar{X}_1 - \bar{X}_2 - 0}{s_p\sqrt{\frac{1}{n_1} + \frac{1}{n_2}}}$$

Where $s_p^2 = \frac{(n_1-1)s_1 + (n_2-1)s_2}{n_1 + n_2 - 2}$

```{r}
s_p <- sqrt(29*10.5 + 29*10.2)
34/(s_p*(sqrt(2/29)))
1-pt(q = 34/(s_p*(sqrt(2/29))), df = 29*2)
```
]
---
...
```{r qqplot and residuals, echo=FALSE, fig.dim=c(7, 3.5)}
chick_residuals <- as_tibble(ChickWeight) %>% 
   mutate(resid = chick_lm$residuals) %>% 
   mutate(prediction = predict(chick_lm))
chick_qqplot <- ggplot(chick_residuals, aes(sample = (resid - mean(resid))/sd(resid))) + 
   geom_qq() + 
   theme_bw() + 
   ggtitle("QQ plot of residuals")
chick_residuals_plot <- ggplot(chick_residuals, aes(x = prediction, y = resid)) + 
   geom_jitter() +
