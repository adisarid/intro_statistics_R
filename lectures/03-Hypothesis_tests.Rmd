...
--
   ...
   * The bias-variance decomposition of point estimates (פירוק שונות-הטיה)
   
   * The grammar of graphics, `ggplot2` (יצירת תרשימים)

   * Confidence intervals (רווחי בר סמך), e.g.:
   
$$P(\bar{X} - z_{\alpha/2}\frac{\sigma}{\sqrt{n}} < \mu < \bar{X} + z_{\alpha/2}\frac{\sigma}{\sqrt{n}}) = 1-\alpha$$

--

   * We've seen confidence intervals (רווח בר סמך) for
   
      * Normal distribution (for $\mu$)
      
      * Using Student's $T$ (unknown variance)

---

# Today

   * We will continue with intervals for
   
      * Binomial (proportion - פרופורציה)
      
      * Variance (for $\sigma^2$)

      * Setting the sample size according to a confidence interval length
      
      * Prediction Intervals (רווח תחזית)

   * We will also start discussing
   
      * Hypothesis tests (מבחני השערות)

---

# The confidence interval generation process תהליך יצירת רב"ס
...
# Confidence Interval for expectancy with known Variance רב"ס לתוחלת עם שונות ידועה
...
# Confidence Interval for expectancy with unknown variance רב"ס לתוחלת, שונות לא ידועה

In this case, we use our estimator $S$ to compute our statistic and confidence interval.

$$T = \frac{\bar{X}-\mu}{S/\sqrt{n}}$$

The statistic $T$ has a student's t-distribution with $n-1$ degrees of freedom (סטטיסטי t). I.e.:

$$P(-t_{\alpha/2,n-1} < T < t_{\alpha/2,n-1})=1-\alpha$$

---

# Determining the sample size from a desired confidence range קביעת גודל מדגם
...
# One sided versus two sided confidence intervals רב"ס חד-צדדי לעומת דו-צדדי
...
# General method to drive a confidence interval תהליך ליצירת רב"ס
...
# CI for variance and standard deviation of a normal population 

רב"ס לשונות וסטיית תקן, אוכלוסיה נורמלית
...
# Margin of error versus sample size מרווח טעות וגודל מדגם
...
# Prediction interval (1/3) - רווח תחזית
...
# Hypothesis testing מבחני השערות
...
The $H_0$ is called the *null hypothesis* (השערת האפס) and the $H_1$ is called the *alternative hypothesis* (השערה אלטרנטיבית).
...
.tiny[
```{r binomial distribution and criteria, echo = FALSE, fig.dim=c(4.5, 3)}
...
# Two types of errors שגיאה מסוג ראשון ושגיאה מסוג שני
...
#### Type-I error טעות מסוג ראשון

The type-I error is also called the *significance level* רמת מובהקות, or $\alpha$-error.
...
# General framework for hypothesis testing תהליך לבנית מבחני השערות
...
# One-tailed versus two-tailed hypothesis מבחן השערות חד-צדדי לעומת דו-צדדי
...
# Hypothesis testing - goodness of fit טיב התאמה
