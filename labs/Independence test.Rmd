...
In the following tutorial we're going to demonstrate the independence test (contingency table) using chi-square.
...
quiz(
  question("What are the top 2 (most common) volcano types?", random_answer_order = T, type = "checkbox",
...
quiz(
  question("What kind of geom would you use to compare the elevation distribution of different volcano types?", random_answer_order = T, type = "checkbox",
...
This yields the observed values $O_i$. Now we turn to computing the expected values. Since we are going to use the `chi.square` function, it is enough to compute the probabilities of each combination primary_volcano_type and elv_gr (if they are independent).

Compute the proportion of mountains in each `elv_gr` and the proportion of mountains in each `primary_volcano_type` (out of the six common types). Make sure that you finish with just two columns in each case (proportion and the `primary_volcano_type` or `elv_gr`)
...
The last step is about computing the probability under the null hypothesis (of independence). It is currently not included in the code, you should add it as part of the exercise, with a `mutate` command.
...
The final step is to use the chi-square test on our contingency. We can use the `contingency_table` we defined in the previous steps. Specifically we need to use `contingency_table$observed`, and `contingency_table$prop`. 

Complete this chi square test to finish the exercise and reveal the answer:

```{r chi_sqaure_test, exercise=TRUE}
chisq.test(...)
```

```{r chi_sqaure_test-hint-1, exercise=TRUE}
chisq.test(x = ..., p = ...)
```

```{r chi_sqaure_test-hint-2, exercise=TRUE}
chisq.test(x = contingency_table$..., p = contingency_table$...)
```

```{r chi_sqaure_test-solution, exercise=TRUE}
