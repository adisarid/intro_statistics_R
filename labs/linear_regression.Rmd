...
First, let's read the data and get familiar with it. In [this link](https://github.com/rfordatascience/tidytuesday/blob/master/data/2020/2020-01-28/readme.md) you will find the data set we will be working with. It contains many trees spread over the streets of San Francisco. Read it, `glimpse` on it. 

You will notice that there is a variable called `date`, discuss in the group:

   * What is the meaning of `date`? 
   * There are some missing values in it - what should be done with them?
   * How would you compute the tree's age? (add this to the code as a new variable `age`)
   * There are some missing values in `dbh` (the diameter variable). Filter these out.

```{r initial_read, exercise=TRUE}

sf_raw <- read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2020/2020-01-28/sf_trees.csv')
...
sf_trees <- sf_raw %>% 
  filter(!is.na(date) & !is.na(dbh)) %>% 
  mutate(age = 2020 - ...) # consider lubridate::year()
...
sf_trees <- read_csv('data/sf_trees.csv') %>% 
  filter(!is.na(date) & !is.na(dbh)) %>% 
  mutate(age = 2020 - lubridate::year(date))
```
